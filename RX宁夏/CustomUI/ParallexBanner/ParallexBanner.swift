...
public class ParallexBanner: UIView {
    ...
    /// The page Control
    public  let pageControl:UIPageControl = AdaptionPageContol()
...
//        pageControl.center = CGPoint(x: self.bounds.width/2, y: self.bounds.size.height - pageControl.bounds.height / 2 + 8.0)
        pageControl.frame = CGRect(x: self.bounds.width - 80, y: self.bounds.height - 29.5, width: 80, height: 29.5)
...
public class BannerCell:UICollectionViewCell{
    ...
    private func commonInit(){
        ...
        imageView.insertSubview(coverView, belowSubview: textLabel)
//        imageView.insertSubview(coverView, aboveSubview: textLabel)
    ...
    override public func layoutSubviews() {
        ...
        textLabel.frame = CGRect(x: 10,
                                 y: self.bounds.height - 29.5,
                                 width: self.bounds.width - 10 - 80,
                                 height: 29.5)
        coverView.frame = CGRect(x: 0, y: self.bounds.height - 29.5, width: self.bounds.width, height: 29.5)
    }
    public func config(item:BannerItem){
//        imageView.image = item.image
        YDWebImageTool.setImage(with: item.imageUrl!, needsSetImageView: imageView)
...
public class BannerItem{
    ...
    var title:String
    var color:UIColor
    init(imageUrl:String?, title:String, color:UIColor) {
...
public class AdaptionPageContol: UIPageControl {
    public override func layoutSubviews() {
        super.layoutSubviews()
        
        let dotw: CGFloat = self.frame.size.width / CGFloat((self.subviews.count*2+1))
        
        let  marginX = dotw * 2
        
        for (i,view) in self.subviews.enumerated() {
            if i == self.currentPage {
                view.frame = CGRect(x: CGFloat(i) * marginX + 5, y: view.frame.origin.y - 1, width: dotw+0.5, height: dotw+0.5)
            } else {
                view.frame = CGRect(x: CGFloat(i) * marginX + 5, y: view.frame.origin.y, width: dotw, height: dotw)
            }
        }
    }
}
